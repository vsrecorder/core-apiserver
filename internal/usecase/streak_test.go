...
func TestStreak_GetByUserId(t *testing.T) {
	...
	t.Run("正常系_先週1週だけ未記録でフリーズ未使用ならまだ継続扱い", func(t *testing.T) {
		mockCtrl := gomock.NewController(t)
		userStreakRepo := mock_repository.NewMockUserStreakInterface(mockCtrl)
		u := NewStreak(userStreakRepo)
	...
	t.Run("正常系_2週続けて未記録でもフリーズが2つ残っていれば継続扱いで2つ消費済みとして返す", func(t *testing.T) {
		mockCtrl := gomock.NewController(t)
		userStreakRepo := mock_repository.NewMockUserStreakInterface(mockCtrl)
		u := NewStreak(userStreakRepo)

		// 8/10 週が最後(フリーズ使用済み1=残り2)、8/17・8/24 週は未記録、今日は 8/31(月)。
		// 空白2週は残り2つでちょうど埋められるので途切れず、次に記録した時点で消費される
		// 2つを先に減らして返す(フリーズが2つ残っているのに途切れて見えていた事例)。
		overrideTimeNow(t, time.Date(2026, 8, 31, 10, 0, 0, 0, time.Local))
		stored := entity.NewUserStreak("user-1", 6, 6, 1, 1, time.Date(2026, 8, 10, 0, 0, 0, 0, time.Local), time.Now())
		userStreakRepo.EXPECT().FindByUserId(gomock.Any(), "user-1").Return(stored, nil)
	...
	t.Run("正常系_記録削除で最終記録週が3週前まで戻った場合はDB由来のUTC日付でも終了扱いにする", func(t *testing.T) {
		...
		// 8/3 週まで5週連続(フリーズ残り1)の人が 8/24 週に記録すると空白2週を埋められず1週目に
		// リセットされるが、その 8/24 の記録を削除すると再計算で「8/3 週を最終週とする5週連続」に戻る。
		// 今日(8/30)から見れば既に途切れているので、表示上は0週・フリーズ未使用で返す。
		// last_recorded_week は DATE カラムのため UTC の 0時 として読み出される点を再現する。
		overrideTimeNow(t, time.Date(2026, 8, 30, 23, 0, 0, 0, time.Local))
		stored := entity.NewUserStreak("user-1", 5, 5, StreakMaxFreezeCount-1, 0, time.Date(2026, 8, 3, 0, 0, 0, 0, time.UTC), time.Now())
		userStreakRepo.EXPECT().FindByUserId(gomock.Any(), "user-1").Return(stored, nil)
	...
	t.Run("正常系_2週続けて未記録でフリーズが1つしか残っていなければ終了扱い", func(t *testing.T) {
		mockCtrl := gomock.NewController(t)
		userStreakRepo := mock_repository.NewMockUserStreakInterface(mockCtrl)
		u := NewStreak(userStreakRepo)

		// 3週前が最後(空白2週)で残り1 → 埋め切れないので0週に戻す
		lastWeek := mondayOf(time.Now()).AddDate(0, 0, -21)
		stored := entity.NewUserStreak("user-1", 2, 2, StreakMaxFreezeCount-1, 0, lastWeek, time.Now())
		userStreakRepo.EXPECT().FindByUserId(gomock.Any(), "user-1").Return(stored, nil)
...
func TestIsStreakExpired(t *testing.T) {
	...
	t.Run("正常系_3週前が最後(空白2週)でも残りフリーズ2つで埋められれば期限切れではない", func(t *testing.T) {
		lastWeek := mondayOf(time.Now()).AddDate(0, 0, -21)
		require.False(t, isStreakExpired(lastWeek, StreakMaxFreezeCount-2))
	})

	t.Run("正常系_3週前が最後(空白2週)で残りフリーズが1つなら期限切れ", func(t *testing.T) {
		lastWeek := mondayOf(time.Now()).AddDate(0, 0, -21)
		require.True(t, isStreakExpired(lastWeek, StreakMaxFreezeCount-1))
	})

	t.Run("正常系_フリーズ未使用でも上限を超える週数あくと期限切れ", func(t *testing.T) {
		lastWeek := mondayOf(time.Now()).AddDate(0, 0, -7*(StreakMaxFreezeCount+2))
	...
	// 3週前が「2週前」に見えて空白1週(残り1で埋められる)と誤判定していた(記録削除で最終記録週が
	// 過去へ戻った直後に、既に途切れているはずのストリークがフリーズ付きで復活して見える)。
	t.Run("正常系_DB由来のUTC日付でも3週前(空白2週)で残り1なら期限切れ", func(t *testing.T) {
		overrideTimeNow(t, time.Date(2026, 8, 30, 23, 0, 0, 0, time.Local)) // 日曜(今週の月曜は 8/24)
		require.True(t, isStreakExpired(time.Date(2026, 8, 3, 0, 0, 0, 0, time.UTC), StreakMaxFreezeCount-1))
	})
