...
func test_OfficialEventController_Get(t *testing.T) {
	...
	t.Run("正常系_クエリ未指定なら当日のイベント一覧を返す", func(t *testing.T) {
		...
		require.WithinDuration(t, startDate, res.StartDate, time.Second)
		require.WithinDuration(t, endDate, res.EndDate, time.Second)
	...
	t.Run("正常系_期間指定で絞り込める", func(t *testing.T) {
		...
		require.Equal(t, typeId, res.TypeId)
		require.Equal(t, leagueType, res.LeagueType)
		require.WithinDuration(t, expectedStartDate, res.StartDate, time.Second)
		require.WithinDuration(t, expectedEndDate, res.EndDate, time.Second)
		require.Equal(t, len(officialEvents), res.Count)
		require.Equal(t, uint(606466), res.OfficialEvents[0].ID)
		require.Equal(t, uint(630879), res.OfficialEvents[1].ID)
	})
