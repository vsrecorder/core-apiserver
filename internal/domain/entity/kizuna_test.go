...
func TestCalculateKizuna(t *testing.T) {
	tests := map[string]struct {
		...
		"大型大会まで連れて行った主力": {
			...
			// 語り度は対戦メモ基準。38対戦にメモ、対戦は60なので記入率63%（記録38件基準の
			// 100%ではない）。narrative 31→20、レベル 227→216 になる。
			wantLevels: map[string]int{"d1": 216, "d2": 64},
			wantPoints: map[string]map[KizunaMetricKey]int{
				"d1": {
