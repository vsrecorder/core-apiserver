...
func test_UserStatRecentInfrastructure_FindRecentMatches(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(
		`SELECT matches.id AS match_id, records.event_date AS event_date, records.official_event_id AS official_event_id, records.deck_id AS deck_id, matches.opponents_deck_info AS opponents_deck_info, matches.victory_flg AS victory_flg, matches.draw_flg AS draw_flg FROM "matches" JOIN records ON records.id = matches.record_id AND records.deleted_at IS NULL AND records.ignore_stats_flg = false WHERE matches.user_id = $1 AND matches.deleted_at IS NULL ORDER BY records.event_date DESC, matches.created_at DESC LIMIT $2`,
	)).WithArgs(
		userId,
		count,
	).WillReturnRows(rows)
...
func test_UserStatRecentInfrastructure_FindRecentMatchesWithDeckId(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(
		`SELECT matches.id AS match_id, records.event_date AS event_date, records.official_event_id AS official_event_id, records.deck_id AS deck_id, matches.opponents_deck_info AS opponents_deck_info, matches.victory_flg AS victory_flg, matches.draw_flg AS draw_flg FROM "matches" JOIN records ON records.id = matches.record_id AND records.deleted_at IS NULL AND records.ignore_stats_flg = false WHERE (matches.user_id = $1 AND matches.deleted_at IS NULL) AND records.deck_id = $2 ORDER BY records.event_date DESC, matches.created_at DESC LIMIT $3`,
	)).WithArgs(
		userId,
		deckId,
		count,
