...
- 表示用の並びは、その組み合わせで**最も多く使われた並び**(代表の並び)を採る。
  同じ組み合わせを1体目と2体目を逆に登録した票が混ざるため、元データの `position` 順を
  そのまま採ると、行のアイコンの左右が「最初に集計された票」次第で決まってしまう
  ([deck-usage-grouping-by-first-sprite.md](deck-usage-grouping-by-first-sprite.md) D6)
- **指紋は表示スロット(position 1/2)のスプライトに限定**する。UI は2枠しか表示しないため、
  3体目以降を含めると「画面に現れないスプライトが指紋だけを分け、見た目が同じ行が複数並ぶ」
  不整合が起こる(表示と集計の単位を一致させる)。エイリアス辞書のロードと自動生成の
  教師データ・代表構成も同じく position 1/2 に限定する
...
### D4b. 前週比較はサーバ側で同じ集計を2回走らせて付与する

順位・使用率・勝率の前週比(▲▼/NEW 表示)のため、リクエスト時に前週 `[from-7d, from)` も
同じ規則で集計し、指紋で突き合わせて `previous_rank` / `previous_usage_rate` /
`previous_win_rate` を変種に付与する(前週に個別表示されていなければ null = NEW)。

- **サーバ側で行う理由**: 順位・「その他」集約・推測フォールバックの規則を1箇所に保つため
  (フロントが2週ぶん取得して再実装すると規則の二重管理になる)。集計コストは約2倍だが、
  キャッシュ無しの現行設計でも許容範囲
- 突き合わせは**指紋(スプライトの組み合わせ)**で行う。前週「その他」の内訳にいた変種も
  比較対象で、内訳の連番を順位として引き継ぐ(前週のUIがその番号を表示しているため)。
  NEW になるのは前週に一度も現れなかった指紋だけ
- 「その他」行は使用率・勝率のみ比較する(順位を持たない)
...
| 軸 | 既定 | 測っているもの | 落とすケース |
|---|---|---|---|
| 支持(最頻構成の件数) | 3件以上 | データ量が足りているか | 1〜2件の共起 = 偶然かもしれない |
| 占有率(最頻構成 ÷ その名前の全件) | 20%以上 | 名前が構成を絞れているか | 「ルギア」が6構成以上に散る = 名前が曖昧 |
| 実ユーザー数(重複なし) | 2人以上 | 1人の命名癖でないか | 1人が100試合記録して支持を作る |

既定は**救済できる票を優先する側に倒してある**。`source='auto'` は実行のたびに総入れ替え
され、誤りは手動エントリで上書きできる(§D9)ため、取りこぼしより拾いすぎを許す。厳しくする
ときは「誤った推測が混じるなら占有率、母数の薄い候補が気になるなら支持」の順に上げる。

マッチ単位で数えるため1人が支持を膨らませられる。これを人数の下限で防ぐ考え方は、D3の `minVariantCount` と同じ匿名化の思想である。

代表スプライトは最頻指紋の構成をそのまま採る(`position` の割り当ても最頻のものを採用)。**これにより推測票が実スプライトの変種と同じ指紋に合流し、後述の分裂が緩和される。** なお D2 に合わせ、教師データの指紋・代表構成とも position 1/2 のみを使う(3体目以降は無視)。
...
| `-min-ratio` | 0.2 | 代表構成の占有率の下限(**割合で指定**) |
| `-min-contributors` | 2 | 代表構成を使った実ユーザー数の下限 |
| `-min-alias-runes` | 2 | 生成するエイリアスの最小文字数(手動運用の下限と同じ。2未満は突合時に捨てられる) |
