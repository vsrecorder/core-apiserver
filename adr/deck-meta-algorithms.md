...
変種が1件でもあれば、前週 `[from-7d, from)` を**同じ規則で**集計し、指紋で突き合わせて
各変種に `previous_rank` / `previous_usage_rate` / `previous_win_rate` を付与する。

- 前週の順位は**個別表示された変種のみ**の連番(「その他」は順位を持たない)
- 前週に個別表示されていない変種(圏外・「その他」集約・新登場)は比較なし → UI では **NEW**
- 「その他」行同士は使用率・勝率のみ比較する。内訳(Members)には付与しない
