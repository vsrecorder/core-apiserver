...
func TestWeeklyReportNotifier_NotifyUser(t *testing.T) {
	...
	t.Run("正常系_1戦以上あればレポートへ誘導する通知を作成する", func(t *testing.T) {
		...
		userStatRepo.EXPECT().FindUserStat(gomock.Any(), "user-1", statPeriodOf(fromDate, toDate), uint(0), false).Return(stat, nil)
	...
	t.Run("正常系_dryRunなら対象判定だけ行い通知は作らない", func(t *testing.T) {
		...
		m.userStat.EXPECT().FindUserStat(gomock.Any(), "user-1", statPeriodOf(fromDate, toDate), uint(0), false).Return(stat, nil)
	...
	t.Run("異常系_戦績集計のエラーはそのまま返す", func(t *testing.T) {
		...
		userStatRepo.EXPECT().FindUserStat(gomock.Any(), "user-1", statPeriodOf(fromDate, toDate), uint(0), false).Return(nil, errors.New("db down"))
...
func TestWeeklyReportNotifier_NotifyUser_EnvNews(t *testing.T) {
	...
	expectEnvNewsPreconditions := func(m *weeklyReportMocks) {
		m.userStat.EXPECT().FindUserStat(gomock.Any(), "user-1", statPeriodOf(fromDate, toDate), uint(0), false).Return(zeroStat, nil)
	...
	t.Run("正常系_一度も記録していない(ストリーク行なし)購読者にも環境ニュースを送る", func(t *testing.T) {
		m, u := setup4WeeklyReportNotifierWithPush(t)
		m.userStat.EXPECT().FindUserStat(gomock.Any(), "user-1", statPeriodOf(fromDate, toDate), uint(0), false).Return(nil, apperror.ErrRecordNotFound)
		m.pushSubscription.EXPECT().FindLiveByUserId(gomock.Any(), "user-1").Return(live, nil)
		m.notification.EXPECT().ExistsByUserIdAndCategoryAndLinkUrl(gomock.Any(), "user-1", NotificationCategoryEnvNews, wantLink).Return(false, nil)
	...
	t.Run("正常系_環境データは週ごとに1回だけ集計し複数ユーザーで使い回す", func(t *testing.T) {
		...
		for _, uid := range []string{"user-1", "user-2"} {
			m.userStat.EXPECT().FindUserStat(gomock.Any(), uid, statPeriodOf(fromDate, toDate), uint(0), false).Return(entity.NewUserStat(uid, 0, 0, 0, 0, 0, 0, 0, 0), nil)
