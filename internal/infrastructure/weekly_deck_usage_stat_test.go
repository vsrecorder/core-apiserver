...
var weeklyMatchRowColumns = []string{"match_id", "user_id", "deck_id", "victory_flg"}
...
func TestWeeklyDeckUsageStatInfrastructure(t *testing.T) {
	...
	expectWeeklyMatchQuery := func(mock sqlmock.Sqlmock) *sqlmock.ExpectedQuery {
		return mock.ExpectQuery(`SELECT matches\.id AS match_id, records\.user_id AS user_id, records\.deck_id AS deck_id, matches\.victory_flg AS victory_flg FROM "matches" JOIN records`).
	...
	t.Run("正常系_スプライト未付与のマッチのみなら変種なしで貢献者数だけ返す", func(t *testing.T) {
		db, mock := setupSqlmockDB(t)
		r := NewWeeklyDeckUsageStat(db)
