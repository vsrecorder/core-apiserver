...
5. 組み合わせごとに**代表の並び**(下記)を決める。集計単位が `first_sprite` なら、
   代表の並びの**先頭1体だけ**に絞って指紋を作り直す(§1.4.1)

表示用のスプライト列は、その組み合わせの代表の並び(position ASC・ID重複は排除済み)。

#### 代表の並び — 同じ組み合わせで1体目と2体目が逆の票

同じ組み合わせでも、票によってどちらを1体目に置いたかは揃っていない
(「リザードン＋ピジョット」と「ピジョット＋リザードン」)。そこで週の全票を一度ためてから、
組み合わせごとに**最も多く使われた並びを代表**として選び、その並びに揃えて数える
(`spriteOrderTally`)。並びのキーは `ID#position` をカンマ連結したもので、票数が同数なら
そのキーの辞書順で決める(票の到着順に左右されず、同じ週を集計し直しても同じ結果になる)。

- `exact`: 件数は元から並び順に依存しない。**行に出すスプライトの並び**が代表の並びになる
  (従来は「最初に集計された票」の並びだった)
- `first_sprite`: **代表の並びの1体目**で束ねる。少数派の並びで登録された票も、
  同じ組み合わせである以上は多数派の行に入る(1体目違いで行が割れない)
...
| 値 | 同一視の条件 | 指紋 | 表示スプライト |
|---|---|---|---|
| `exact`(既定) | position 1/2 のスプライト集合が一致 | ID集合をソートしてカンマ連結 | 1〜2体(代表の並び) |
| `first_sprite` | **代表の並びの1体目が同じ** | 1体目のID | **1体だけ**(position は 1 に揃える) |

- 「1体目」は `position == 1` ではなく **position ASC で並べた先頭**。1枠目が欠けて
  2枠目だけに登録されている票(旧データ)を、指紋を作れない票として落とさないため
- 束ねるのは票ごとの1体目ではなく**代表の並びの1体目**(§1.4)。同じ組み合わせを
  1体目違いで別の行にしないため
...
変種が1件でもあれば、前週 `[from-7d, from)` を**同じ規則で**集計し、
**指紋(スプライトの組み合わせ)**で突き合わせて各変種に
`previous_rank` / `previous_usage_rate` / `previous_win_rate` を付与する。

- 前週の順位は個別表示された変種の連番。**前週「その他」の内訳(Members)にいた変種も
  比較対象**で、内訳の連番(「その他」行の次から)を順位として引き継ぐ。
  前週のUIが内訳をその番号で表示しているため、見た目と比較が一致する
