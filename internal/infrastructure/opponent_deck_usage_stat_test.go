...
func test_OpponentDeckUsageStatInfrastructure_FilterByDeckIdUsesRecordsDeckId(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(
		`SELECT matches.id AS match_id, matches.opponents_deck_info AS deck_info, matches.victory_flg AS victory_flg, matches.draw_flg AS draw_flg FROM "matches" JOIN records ON matches.record_id = records.id WHERE (records.user_id = $1 AND records.deleted_at IS NULL AND records.ignore_stats_flg = false AND matches.deleted_at IS NULL AND matches.opponents_deck_info != '') AND records.deck_id = $2 ORDER BY records.event_date ASC`,
