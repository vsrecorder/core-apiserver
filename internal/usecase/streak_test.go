...
func TestStreak_GetByUserId(t *testing.T) {
	t.Run("記録が無いユーザーは0件のストリークを返す", func(t *testing.T) {
	...
	t.Run("直近の記録から1週間以内ならそのまま返す", func(t *testing.T) {
		mockCtrl := gomock.NewController(t)
		userStreakRepo := mock_repository.NewMockUserStreakInterface(mockCtrl)
		u := NewStreak(userStreakRepo)
	...
	t.Run("フリーズ猶予(2週間)ちょうどでフリーズ未使用ならまだ継続扱い", func(t *testing.T) {
		mockCtrl := gomock.NewController(t)
		userStreakRepo := mock_repository.NewMockUserStreakInterface(mockCtrl)
		u := NewStreak(userStreakRepo)
	...
	t.Run("記録の作成・削除以来、時間経過だけでフリーズ猶予を超えた場合は表示上0に戻す", func(t *testing.T) {
		mockCtrl := gomock.NewController(t)
		userStreakRepo := mock_repository.NewMockUserStreakInterface(mockCtrl)
		u := NewStreak(userStreakRepo)
	...
	t.Run("フリーズ猶予(2週間)を超え、かつフリーズ使用済みなら1週間経過時点でも終了扱い", func(t *testing.T) {
		mockCtrl := gomock.NewController(t)
		userStreakRepo := mock_repository.NewMockUserStreakInterface(mockCtrl)
		u := NewStreak(userStreakRepo)
...
func TestIsStreakExpired(t *testing.T) {
	t.Run("記録が一度も無い(ゼロ値)場合は期限切れ扱いにしない", func(t *testing.T) {
		require.False(t, isStreakExpired(time.Time{}, 0))
	})

	t.Run("先週記録していれば今週分の記録がまだでも期限切れではない", func(t *testing.T) {
		lastWeek := mondayOf(time.Now()).AddDate(0, 0, -7)
		require.False(t, isStreakExpired(lastWeek, 0))
	})

	t.Run("3週間以上前で猶予を超えると期限切れ", func(t *testing.T) {
		lastWeek := mondayOf(time.Now()).AddDate(0, 0, -21)
