...
func TestUnofficialEventUpdateMiddleware(t *testing.T) {
	marshal := func(t *testing.T, req dto.UnofficialEventUpdateRequest) string {
		t.Helper()
		b, err := json.Marshal(req)
		require.NoError(t, err)
		return string(b)
	...
	t.Run("正常系_イベント名と開催日が揃ったリクエストを受理して設定する", func(t *testing.T) {
		expected := dto.UnofficialEventUpdateRequest{
			UnofficialEventRequest: dto.UnofficialEventRequest{
				Title: "身内対戦会",
				Date:  time.Date(2026, 7, 18, 0, 0, 0, 0, time.UTC),
			},
		...
		UnofficialEventUpdateMiddleware()(ctx)

		require.Equal(t, http.StatusOK, w.Code)
		require.Equal(t, expected, helper.GetUnofficialEventUpdateRequest(ctx))
	})
