...
func TestNotificationInfrastructure(t *testing.T) {
	...
	t.Run("FindByUserIdAndCategoryAndBodies", func(t *testing.T) {
		bodies := []string{
			"2026シーズンで「週次記録3週連続」バッジを獲得しました！",
			"2026シーズンで「週次記録7週連続」バッジを獲得しました！",
		}

		t.Run("正常系_categoryとbodyが完全一致する通知を返す", func(t *testing.T) {
			db, mock := setupSqlmockDB(t)
			r := NewNotification(db)

			mock.ExpectQuery(regexp.QuoteMeta(
				`SELECT * FROM "notifications" WHERE user_id = $1 AND category = $2 AND body IN ($3,$4) ORDER BY created_at DESC, id DESC`,
			)).WithArgs(
				uid, "streak", bodies[0], bodies[1],
			).WillReturnRows(sqlmock.NewRows(notificationColumns).AddRow(
				id, createdAt, uid, "streak", "ストリークを継続中です", bodies[1], "/users", false, nil,
			))

			ret, err := r.FindByUserIdAndCategoryAndBodies(context.Background(), uid, "streak", bodies)

			require.NoError(t, err)
			require.Len(t, ret, 1)
			require.Equal(t, id, ret[0].ID)
			require.Equal(t, bodies[1], ret[0].Body)
			require.NoError(t, mock.ExpectationsWereMet())
		})

		t.Run("正常系_bodiesが空ならSQLを発行せず空を返す", func(t *testing.T) {
			db, mock := setupSqlmockDB(t)
			r := NewNotification(db)

			// ExpectQueryを一切設定していないため、SELECTが発行されればエラーになる。
			// 条件が緩んでそのユーザの通知を全件拾ってしまうのを防ぐガードの確認。
			ret, err := r.FindByUserIdAndCategoryAndBodies(context.Background(), uid, "streak", nil)
	...
	t.Run("DeleteByIds", func(t *testing.T) {
		t.Run("正常系_指定したIDの通知を削除する", func(t *testing.T) {
			db, mock := setupSqlmockDB(t)
			r := NewNotification(db)

			ids := []string{id, "01HD7Y3K8D6FDHMHTZ2GT41TN3"}
			...
			require.NoError(t, r.DeleteByIds(context.Background(), ids))
			require.NoError(t, mock.ExpectationsWereMet())
		})

		t.Run("正常系_idsが空ならSQLを発行しない", func(t *testing.T) {
			db, mock := setupSqlmockDB(t)
			r := NewNotification(db)

			// ExpectExecを一切設定していないため、DELETEが発行されればエラーになる。
			// WHERE条件が消えて全件削除になるのを防ぐガードの確認。
