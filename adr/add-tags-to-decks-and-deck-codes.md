...
- 稼働DBへは `db/migration_add_record_tags.sql`(列追加・既存プリセットの `acespec` 寄せ・順位プリセット投入・`record_tags` 作成)を流した。**2026-08-31 適用済み・ファイルは削除済み**(4節・7節と同じ運用)。内容が必要になったら git の履歴(コミット `9b2e1df`)から参照する。
